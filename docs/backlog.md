...
- [~] **Carrier adjudication desk — persona/reserve/polarity review + fixes (Session 2026-06-18) — SHIPPED (one open ops item).** Engineering-lead deep-dive of the carrier claims-adjudication surface (`/adjusting/[cid]`, persona "Sam Rivera · carrier") → spec [`2026-06-17-carrier-adjudication-persona-fixes-design.md`](superpowers/specs/2026-06-17-carrier-adjudication-persona-fixes-design.md) + plan [`2026-06-17-carrier-adjudication-persona-fixes.md`](superpowers/plans/2026-06-17-carrier-adjudication-persona-fixes.md); executed subagent-driven (impl + review per task). Shipped F-1…F-11:
  - **F-7 (`1198017`)** coerce packet JSON (`risk_signals`/`memo`/`citation_ids`) at the read boundary in `_incident_report` — AI panel no longer silently vanishes on Postgres (the Neon JSON-string class).
  - **F-4 (`257aa6e`)** `decide_coverage("denied")` no longer auto-closes — records the denial, leaves the claim **open** at `under_investigation`; closing is a separate explicit action (matches dispute/appeal). Rewrote the deny test.
  - **F-3 (`7ab3652`)** ungated reserve at FNOL — removed the coverage-first lock on the action grid; reserve/close/expense/recovery live from `notified`; only **indemnity** payment stays coverage-gated (server-enforced). Backend already supported reserve-at-notice; the UI was contradicting it.
  - **F-2 (`e16af07` → fix `88c1ff2`)** reframed the recommendation block for the risk-bearer: "Net EV" → "Indemnity exposure (EV)", tone by magnitude not sign, memo → "Insured's defense posture". **Review caught** that `net_expected_value_usd` is the *insured's* metric (`carrier_payout − premium impact`); corrected to show **`carrier_payout × probability`** (the true carrier exposure; $4,050 on the demo claim).
  - **F-1 (`f3027e5`)** route-aware persona label — sidebar reads "Carrier · Claims" under `/adjusting`, "Carrier · Underwriting" elsewhere (one `carrier` role does both; no new role/login — vertically-integrated model).
  - **F-8/F-9 (`148263d` helper, `03fbe03` render, `7a5a981` fallback)** `reserveAdequacy` helper (pure, vitest) + `chain_ladder_mean` on `ReserveHint` → hero "Reserve adequacy" cell + chain-ladder line in the hint banner. Context-aware: advisory band pre-payment, incurred delta after; **when the band collapses to $0–$0 (thin/zero-incurred history) falls back to the model's expected median** ("Below expected payout ($7,000)").
  - **F-5 closed — not a bug** (`settling` is reachable via first indemnity payment, `record_payment`). F-6/F-10/F-11 also addressed (F-10 below).
  - **F-10 (`ebb8150`)** `file_fnol` rejects a `date_of_loss` after today (guard after the policy-term check); capped `seed_defense_demo` loss_date at today.
  - **F-11 (`5330872`)** adjuster queue resolves venue names DB-first (matched the detail route; was VENUES-seed-only).
  - **Seed tooling (`5d12943`)** `seed_defense_demo --refresh` deletes the stale `inc-defense-demo` artifacts (Claim→citations→packet→sources→audit→evidence→incident, children-first for PG FK) and recreates with a capped date, **plus** seeds a closed prior liquor loss so the venue loss run (hence the advisory band) is non-zero.
  - Gates: backend **1604 passed** (lone red = pre-existing `test_evals_baseline` order flake, passes in isolation); frontend **89 vitest**; `tsc` clean. Verified live via the prod carrier API (exposure $4,050, incident panel renders). Global `~/.claude/CLAUDE.md` gained a **Workflow Discipline** rule (brainstorm before code; "refine prompt to expert level" = a checkpoint).
  - [ ] **PICK UP HERE — refresh prod demo data** (ops, not code): the live claim `clm-00faaf1d8e98` still shows a future loss date (`2026-07-29`) + `$0–$0` advisory band because the row predates the date-cap and seeds skip existing rows. From `backend/`, with the **Neon public** URL (runs kept hitting local SQLite when the env var was unset): `$env:DATABASE_URL="<DATABASE_PUBLIC_URL>"; python -m scripts.seed_defense_demo --refresh`. Confirm the `[seed] packet=…/claim=…` line prints; then the date flips to ≤ today and the band goes non-zero. (Code is shipped + verified; this is purely demo-data cosmetics.)
...
- [x] **Session 2026-06-15** — **web design-system migration: "Paper & Ink" on shadcn/21st.dev (foundation + login shipped; sweep IN PROGRESS).** All gates green (design-lint, eslint, vitest, `next build`, **full Playwright E2E**). Commits `0a31d26` (foundation+login), `e48cf51`/`8f3bc36` (E2E selector fixes).
  ...
  - **[ ] PICK UP HERE — continue the sweep, simplest-first:**
    ...
    - [x] **`policy-requests` migrated (Session 2026-06-15).** Broker request queue → inline ds header, ds filter chips (active/count), ds `overflow-x-auto` table, status → ds Badge via `TONE_VARIANT`, approve/decline → ds Buttons; the decline `PromptDialog` (from Phase 3) kept. Gates: design-lint 0/0, ESLint 0 errors, `next build` ○ static.
    - [x] **`coverage` migrated (Session 2026-06-15).** Operator policy view → inline ds header (Caveat "safety net" accent), policy cards → ds Card (carrier eyebrow + status Badge + facts + "Request an action" Button), request-history → ds table; shared `PolicyRequestModal` + the withdraw `ConfirmDialog` kept. Now fully ds (chrome + dialog). Gates: design-lint 0/0, ESLint 0 errors, `next build` ○ static.
    - **Next candidates:** `surplus-lines` (~250), `submissions` kanban, `claims`, `work-queue`, the carrier desk pages (`underwriting`, `adjusting`), `compliance` chrome, `incidents/[id]`, `book`, `venues`, `market`, `report`. **migrate the dashboard LAST.** **migrate the dashboard LAST** (`src/app/dashboard/page.tsx` is ~1,300 lines, dual-persona broker/operator, a phone variant `DashboardMobile`, ticker + triage console — its own focused session; the legacy AppShell sidebar migrates with it).
