...
- **Timestamps:** new `datetime` columns use `Field(default_factory=now_utc, sa_type=DateTimeUTC)` from `app.time`. `DateTimeUTC` stores naive-UTC and reads back tz-aware on both SQLite and Postgres, so timestamps never mix offset-naive/offset-aware (the bug `as_utc` papers over). `datetime.utcnow` is fully migrated out — don't reintroduce it.
